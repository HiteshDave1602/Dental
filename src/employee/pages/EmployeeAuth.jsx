...
const EmployeeAuth = () => {
  ...
  const videoOnRight = isRegistering || isForgot;
  ...
  const changeTab = (nextTab) => {
    ...
    const goesRight = nextTab === 'register' || nextTab === 'forgot';
    setVideoTransition(goesRight ? 'md:animate-auth-video-to-register' : 'md:animate-auth-video-to-login');
    setTab(nextTab);
  ...
  const submitForgot = async (event) => {
    ...
    setLoading(true);
    try {
      await api.employee.auth.forgotPassword({ email: form.email });
      setEmailSent(true);
      notifySuccess('If that email is registered, a reset link has been sent.');
    } catch (err) {
      // The backend always returns a generic message; surface it verbatim so
      // we never leak whether the account exists.
      const msg = err?.response?.data?.detail || 'Unable to send reset link. Please try again.';
      notifyError(typeof msg === 'string' ? msg : JSON.stringify(msg));
    } finally {
      setLoading(false);
    }
  ...
  return (
    <main className="employee-shell relative flex min-h-screen items-center justify-center overflow-hidden bg-[#061638] px-5 py-10 text-white">
      ...
      <section className="relative z-10 w-full max-w-5xl overflow-hidden rounded-[2rem] border border-white/20 bg-white shadow-2xl shadow-black/30 md:min-h-[590px]">
        <aside className={`relative z-20 min-h-[330px] overflow-hidden p-8 shadow-2xl shadow-[#061638]/35 will-change-[left,width,border-radius] md:absolute md:inset-y-0 md:w-1/2 md:p-10 motion-reduce:animate-none ${videoTransition || (videoOnRight ? 'md:left-1/2 md:rounded-l-[48%] md:rounded-r-none' : 'md:left-0 md:rounded-r-[48%]')}`}>
          ...
          <div className="relative h-full">
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
              ...
              <img src={pathfinderLogo} alt="My Pathfinder" className="mx-auto mb-5 h-20 w-20 rounded-2xl bg-white/95 object-contain p-1 shadow-lg" />
                <p className="mb-3 text-sm font-semibold text-[#c1e5ff]">
                  {isRegistering ? 'Already a member?' : isForgot ? 'Trouble signing in?' : 'New to MyPathFinder?'}
                </p>
                <h1 className="employee-heading text-4xl font-bold leading-tight tracking-[-0.04em]">
                  {isRegistering ? 'Welcome back.' : isForgot ? 'Let\u2019s get you back in.' : 'The clarity to plan every smile.'}
                </h1>
                <p className="mt-4 text-sm leading-6 text-white/80">
                  {isRegistering
                    ? 'Sign in to continue your secure dental planning workflow.'
                    : isForgot
                      ? 'Enter your email and we\u2019ll send you a secure password reset link.'
                      : 'Create your account and turn every scan into a confident clinical decision.'}
                </p>
...
<div className={isRegistering ? 'mb-4' : 'mb-8'}>
                ...
                <h2 className={`employee-heading font-bold tracking-[-0.035em] ${isRegistering ? 'text-[28px]' : isForgot ? 'text-[28px]' : 'text-3xl'}`}>
                  {isRegistering ? 'Create your access' : isForgot ? 'Reset your password' : 'Welcome back'}
                </h2>
                <p className={`${isRegistering ? 'mt-1 leading-5' : 'mt-2 leading-6'} text-sm text-[#57718a]`}>
                  {isRegistering
                    ? 'Set up secure access for your planning workspace.'
                    : isForgot
                      ? 'Enter your registered email and we\u2019ll send you a reset link.'
                      : 'Sign in to continue your aligner planning workflow.'}
                </p>
              </div>
              ...
              {isForgot && emailSent ? (
                <div className="space-y-4">
                  <div className="flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3">
                    ...
                    <p className="text-sm text-emerald-800">
                      If an account exists for <span className="font-bold">{form.email}</span>, a reset link is on its way. Check your inbox (and spam folder).
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => { setEmailSent(false); changeTab('login'); }}
                    className="flex h-[52px] w-full items-center justify-center gap-2 rounded-xl bg-[linear-gradient(110deg,#123cc7,#1359d4)] px-5 py-3 font-bold text-white shadow-[0_14px_26px_rgba(18,60,199,0.27)] transition hover:-translate-y-0.5"
                  >
                    <ArrowLeft size={18} />
                    Back to login
                  </button>
                ...
                <form className={isRegistering ? 'space-y-2' : 'space-y-4'} onSubmit={isRegistering ? submitRegister : isForgot ? submitForgot : submitLogin}>
                  ...
                  {!isForgot && <label className="block"><span className={labelClass}>Password</span><div className="relative"><Lock size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#6ab0e3]" /><input className={`${inputClass} pl-12 pr-12`} type={showPassword ? 'text' : 'password'} placeholder="Enter your password" value={form.password} onChange={(e) => setForm((s) => ({ ...s, password: e.target.value }))} required /><button type="button" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? 'Hide password' : 'Show password'} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#7892a8] hover:text-[#123cc7]">{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button></div></label>}
                  ...
                  {!isRegistering && !isForgot && <div className="flex items-center justify-between pt-1 text-sm text-[#57718a]"><label className="flex items-center gap-2"><input type="checkbox" className="h-4 w-4 rounded border-[#b6d8ef] text-[#123cc7] focus:ring-[#73bfed]" />Remember me</label><button type="button" onClick={() => changeTab('forgot')} className="font-bold text-[#123cc7] hover:text-[#0b2b86]">Forgot password?</button></div>}
                  <button type="submit" disabled={(isRegistering && !canRegister) || loading} className="group flex h-[52px] w-full items-center justify-center gap-2 rounded-xl bg-[linear-gradient(110deg,#123cc7,#1359d4)] px-5 py-3 font-bold text-white shadow-[0_14px_26px_rgba(18,60,199,0.27)] transition hover:-translate-y-0.5 disabled:opacity-40"><span>{loading ? 'Please wait...' : isRegistering ? 'Create account' : isForgot ? 'Send reset link' : 'Sign in'}</span><ArrowRight size={18} className="transition-transform group-hover:translate-x-1" /></button>
                </form>
              )}
              <p className={`${isRegistering ? 'mt-4' : 'mt-7'} text-center text-sm text-[#57718a]`}>
                {isRegistering
                ...
                <button
                  type="button"
                  onClick={() => changeTab(isRegistering ? 'login' : 'login')}
                  className="font-bold text-[#123cc7] hover:underline"
                >
                  {isRegistering ? 'Login' : isForgot ? 'Back to login' : ''}
                </button>
              </p>
          </div>
        </section>
